    ...
    for attempt in range(max_retries):
        try:
            ...
            if task == 'generate':
                ...
                # Nest aspect_ratio and/or image_size in image_config per API spec.
                # Each kwarg is included only when set, so omitting both preserves
                # the model's default behavior (1K, model-chosen aspect).
                image_config_kwargs = {}
                if aspect_ratio:
                ...
                if image_config_kwargs:
                    config_args['image_config'] = types.ImageConfig(**image_config_kwargs)
...
Examples:
  ...
  %(prog)s --task generate --prompt "A mountain landscape" \\
    ...
    parser.add_argument('--image-size', choices=['1K', '2K', '4K'],
                       help='Image resolution tier for generation (default: model default 1K). '
                            'Use 2K for full-bleed/background heroes, 1K for contained slots.')
