...
name: ai-multimodal
model: haiku
description: Process and generate multimedia with the Google Gemini API: transcribe/analyze audio, understand images (captioning, OCR, object detection, visual Q&A), analyze video (scene detection, temporal Q&A, YouTube URLs), extract from PDFs (tables, forms, charts, multi-page), and generate/edit images from text. Use for audio/video/image/PDF analysis or multimodal AI features.
license: MIT
allowed-tools:
  - Bash
  - Read
...
> **Environment note (local install):** This skill's dependencies live in a dedicated Python 3.12 virtualenv. Run all `python scripts/...` commands below with that interpreter:
> `~/.claude/skills/ai-multimodal/.venv/bin/python scripts/<name>.py ...`
> Requires `GEMINI_API_KEY` set in the environment (see the API Key section below).
