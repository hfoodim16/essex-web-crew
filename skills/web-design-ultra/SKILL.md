...
**Run the directions against a taste check.** Use `references/taste-planning.md` — the four planning sections, forked in and reconciled with this skill's rules (the standalone `taste-skill` is not invoked; it is build-time material). Take its **§0 Brief Inference** to state a one-line Design Read before generating ("Reading this as: <page kind> for <audience>, with a <vibe> language…"), then check all three directions against its **§0.D Anti-Default Discipline** and **§9 AI Tells**. Take its reasoning, not its picks: where it conflicts with this skill — it suggests Geist, which is on the banned list — **this skill wins**. Precedence is brief → web-design-ultra → taste.
...
- **Builder owns Stages 6–7.** Implements the plan exactly — does **not** re-decide direction, fonts, or palette. Generates every `GENERATE`-marked image (**no count cap**; see `references/imagery.md` crew tier), plus the `VIDEO` slot if the plan marked one **and it was approved**, generated **to its declared register** (`references/video.md` crew tier: one clip per site, ≤8s, never both registers). **All paid generation shares one all-in site budget: $1.00 with no video, $1.50 with one** — this replaced the old per-register ceilings (filmed ≤$1, designed ≤$2.50) on 2026-08-04.
...
| Planning taste: Design Read, anti-default discipline, AI tells, redesign protocol | `references/taste-planning.md` |
| Deterministic plan lint (quotas + required fields, before any HTML) | `scripts/plan-lint.mjs` |
| Deterministic anti-pattern scan (60+ rules, no LLM, ~1s) — run on **every page** | `scripts/detect.mjs` |
