...
Typographic assessment (hierarchy, measure, scale, delivery) paired with a type-scoped mechanical scan.
...
Changing the font pairing itself is a Stage 5 decision — see [directions.md](directions.md); the pairing locked there is the one you improve the use of here.

## Two isolated assessments

Run these in this order, and keep them separate. Do not let detector findings anchor the design assessment.
...
```bash
node ~/.claude/skills/web-design-ultra/scripts/detect.mjs --json --scope type [target files or dirs]
```
