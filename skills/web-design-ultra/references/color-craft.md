...
How to BUILD the palette (roles, OKLCH, contrast). Pair with [color-conventions.md](color-conventions.md), which decides WHICH palette the industry expects.
...
Read the direction chosen in Stage 5, the `:root` tokens, assets, current themes, and representative states. Identify:
...
If a new identity is required, that is a Stage 5 decision — go back to [directions.md](directions.md) and [color-conventions.md](color-conventions.md) rather than inventing one here. Ask only when a binding brand decision cannot be inferred.
