...
Structural assessment (squint test, grouping, rhythm, density) paired with a layout-scoped mechanical scan.

<sub>Adapted for web-design-ultra from the Impeccable skill (github.com/pbakaus/impeccable, Apache-2.0). Native-platform, live-mode, and DESIGN.md-specific material removed; paths and stage handoffs rewritten for this pipeline.</sub>
...
Preserve the direction chosen in Stage 5. This pass changes structure *inside* that direction; replacing the direction itself is a Stage 5 decision — see [directions.md](directions.md).

## Two isolated assessments

Run these in this order, and keep them separate — a clean mechanical scan must not talk you out of a hierarchy problem you can see.
...
```bash
node ~/.claude/skills/web-design-ultra/scripts/detect.mjs --json --scope layout [target files or dirs]
```
...
When the work holds up, it still has to clear **Stage 8** — see [critique.md](critique.md).
