...
- **Crew pitch mockups (Essex Web Crew): no count cap — pre-approved up to the site budget.** The site has one all-in budget covering every paid generation: **$1.00 if it ships no video, $1.50 if it ships one** (`docs/media-policy.md`). At ~$0.04/1K and ~$0.06/2K that is roughly **16 images at 2K or 25 at 1K**, so on a still-only site money is not the binding constraint — design judgment is. The planner marks each `GENERATE` slot with its aspect + resolution tier and carries a **running cost total** that must fit the budget; the builder generates them and stops at the ceiling. **Slots whose real answer is the client's own job photography** — galleries, before/after, the actual crew and trucks — **stay labeled `<!-- AI-IMAGE: … -->` placeholders**, a content-honesty call rather than a cost one. Anything that would break the budget needs the lead to ask Harry.
